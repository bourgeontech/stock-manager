...
$btot3=array();
									...
									foreach($bankledgers as $val){
										$payment1 = $this->db->query("SELECT sum(payment.amount) as payment,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE ledger.led_Id='$val[led_Id]' AND payment.is_delete=0 AND `payment_date` < '$convertDate' and `type`='1'")->row_array();
    
         // print "SELECT sum(payment.amount) as payment,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE `group`='3' AND payment.is_delete=0 AND `payment_date` < '$convertDate' and `type`='1'";
    $pay1=$payment1['payment'];
    
    
    $receipt1 = $this->db->query("SELECT sum(payment.amount) as receipt,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE ledger.led_Id='$val[led_Id]' AND payment.is_delete=0 AND `payment_date` < '$convertDate' and `type`='2'")->row_array();
    $rec1=$receipt1['receipt'];
...
<?php }  }?>
      ...
      //  $this->db->join('ledger_group', 'ledger_group.group_id = ledger.group');
        $this->db->where('payment.payment_date',$convertDate);
        $this->db->where('payment.type', 1);
		$this->db->where('payment.mode', $val['led_Id']);
        $this->db->where('payment.is_delete !=', 1);
        $this->db->order_by("payment.pay_Id", "asc");
