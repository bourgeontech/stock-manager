...
public function getstarByCodeDiety(){
  ...
  /**  public function billing(){
	...
	private function get_last_order_id() {
        // Assuming 'orders' is your table and 'order_id' is the column
        ...
        if ($query->num_rows() > 0) {
            ...
            // If no orders exist yet, return 0.
            return 0;
        }
    }
	...
	public function generate_order_id() {
        ...
        // Get the last order ID from the database or initialize to 0
        $last_order_id = $this->get_last_order_id();
		
        // Increment the counter for the new order ID
        $counter = $last_order_id + 1;

        // Format the order ID
        $order_id = "ORD{$current_date}{$current_time}{$counter}";
...
//         elseif($this->input->post('save')=="print"){
                 ...
                 $result = $till->row_array();
                 $printer = $result['printer'];
        		if($this->input->post('save')=="add_item"){
                	if($dety == '102') {
                    	$type = 2;
                    } else {
                    	$type = 1;
