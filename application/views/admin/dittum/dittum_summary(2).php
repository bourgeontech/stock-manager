				  ...
				  <tbody>
                    ...
                    //total opening for the date 
                        ...
                        foreach($dit as $d){
                        	...
                        	$pids[] = $pid;
                            $dqty = $d['qty'];
                      //  print $dqty;                       print "SELECT SUM(qlt) as qty FROM `billing_dtls` WHERE pooja='$pid' AND `date`='$date'";
                           ...
                           // print "SELECT SUM(qlt) as qty FROM `billing_dtls` WHERE pooja='$pid' AND `date`<'$date'";
                        	$usage +=  ($dqty*$pqty);
                            $total1 += $dqty*$pqty1;
                          //print $date."-".$dqty."-".$pqty1."<br>";
                        }
                    
                   // print $total1."Ditu";
                    //print $usage;
					//	print $opening;print $total1;print $purchase;
                    	$opening_balance = ($opening + $purchase) - ($total1);
                //    print $opening_balance."OB";
                       ...
                       ?> 
                    	<tr>
                        	...
                        	<td><?php echo number_format((float)$opening_balance, 2, '.', '');?></td>
                    	  	<td><?php echo number_format((float)$purchase_qty, 2, '.', '');?></td>
                       		<td><?php echo number_format((float)$excess_inward, 2, '.', '');?></td>
					...
					for ($date=$datef;$date<=$datet;$date=date('Y-m-d', strtotime($date. ' + 1 days'))){
                     ...
                     foreach($products as $product) {
                       ...
                       ?> 
                    	<tr>
                        	<td><?php echo date('d-m-Y',strtotime($date));?></td>
                        	<td><?php echo number_format((float)$opening_balance, 2, '.', '');?>ee</td>
                    	  	<td><?php echo number_format((float)$purchase_qty, 2, '.', '');?></td>
                       		<td><?php echo number_format((float)$excess_inward, 2, '.', '');?></td>
                        	<td><?php echo number_format((float)$total, 2, '.', '');?></td>
                       		<td><?php echo number_format((float)$issue, 2, '.', '');?></td>
