...
//=========
       if ($send_for == "exam_schedule") {
			$this->sendExamSchedule($sender_details);
		}
    ...
    public function sendExamSchedule($sender_details){
		 foreach($sender_details as $val){
                 $detail = array(
                    'email' => $val['guardian_phone'],
					'student_name' => $val['firstname'],
                    );
					$this->CI->mailgateway->sentExamScheduleMail($detail);
			}			  
	}  
