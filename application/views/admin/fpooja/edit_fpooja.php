	  ...
	  <!--</div>-->
       <div class="row">
	     <div class="col-lg-12 col-md-12 col-sm-12 ">		 
	        <div class="shadow-sm p-3 mb-2 bg-white" style="position:relative" >
	        <form action="<?php echo base_url(); ?>index.php/admin/admin/edit_fpooja/<?php echo $f_id;?>" method="post" id="myform">
               <div class="row">
                  <div class="col-lg-3 col-md-3 col-sm-3 ">
                    <h2 class="page_txt"><i class="fa fa-plus-square-o" aria-hidden="true"></i>&nbsp;&nbsp;Edit Family Pooja</h2>
                </div> 
        ...
        <div class="row" style="border:1px solid black;padding:10px;">  
           <div class="col-lg-4">
           	  <div class="form-group">
                  ...
                  <input class="form-control" placeholder="Name" name="fname" value="<?php echo $fpooja_list['name'];?>" type="text">
              </div>
           </div>
           <div class="col-lg-4">
           	  <div class="form-group">
              	  <label class="form-label">Address</label>
                  <input class="form-control" placeholder="Address" name="address" value="<?php echo $fpooja_list['address'];?>" type="text">
              </div>
           </div>
           <div class="col-lg-4">
           	  <div class="form-group">
              	  <label class="form-label">City</label>
                  <input class="form-control" placeholder="City" name="city" value="<?php echo $fpooja_list['city'];?>" type="text">
              </div>
           </div>
           <div class="col-lg-4">
           	  <div class="form-group">
              	  <label class="form-label">Pin Code</label>
                  <input class="form-control" placeholder="Pin Code" name="pincode" value="<?php echo $fpooja_list['pincode'];?>" type="text">
              </div>
           </div>
           <div class="col-lg-4">
           	  <div class="form-group">
              	  <label class="form-label">Mobile</label>
                  <input class="form-control" placeholder="Mobile" name="mobile" value="<?php echo $fpooja_list['mobile'];?>" type="text">
              </div>
           </div>
           <div class="col-lg-4">
           	  <div class="form-group">
              	  <label class="form-label">Email</label>
                  <input class="form-control" placeholder="Email" name="email" value="<?php echo $fpooja_list['email'];?>" type="email">
              </div>
           </div>
        </div>
        <div class="row" style="border:1px solid black;padding:10px;">
              ...
              <div class="form-group">
                  <div class="row_form">
            		...
            		<tbody id="dataTable2">
                    	...
                    	$i=100;
                        $row=100+$row_count;
                    	foreach ($fpoojadtl_list as $dtl){
                    	?>
                		<tr>
                			<td><select name="temple[]" id="temple_<?= $i;?>" onchange="changepooja('<?= $i;?>')" class=" sq_form" required style="width: 3.5cm;">
                                	<option value="">Select Diety</option>
                    			<?php foreach($temple_diety_list as $val){ ?>
                    				<option value="<?= $val['id']; ?>" <?php if($dtl['diety']==$val['id']){ echo "selected";}?>><?= $val['name']; ?></option>
                    			<?php } ?>
                    			</select>
                			</td>
                			<td><input type="text" name="name[]" id="name_<?= $i;?>" class="sq_form" placeholder="Name" value="<?php echo $dtl['name'];?>" required style="width: 3.5cm;"></td>
                			<td><select name="star[]" id="star_<?= $i;?>" class=" sq_form" required style="width: 3.5cm;">
                                	<option value="">Select Star</option>
                    			<?php foreach($birth_star as $val){ ?>  
                    				<option value="<?= $val['id']; ?>" <?php if($dtl['star']==$val['id']){ echo "selected";}?>><?= $val['name_eng']." - ".$val['name_mal']; ?></option>
                    			<?php } ?>
                    			</select>
                			</td>
                			<td><select name="pooja[]" id="pooja_<?= $i;?>" onchange="change_rate('<?= $i;?>')" onfocus="new_f('<?= $i;?>')" onload="change_rate('<?= $i;?>')" class="sq_form" required style="width: 3.5cm;">
                                	<option value="">Select Pooja</option>
                            	<?php foreach($pooja_list as $val){ ?>  
                    				<option value="<?= $val['id']; ?>" <?php if($dtl['pooja']==$val['id']){ echo "selected";}?>><?= $val['name']." - ".$val['name_mal']; ?></option>
                    			<?php } ?>
                    			</select>
                			</td>
                			<td><input type="hidden" value="<?= $row;?>" id="row_value">
                			    <input type="number" min="1" name="qlt[]" onchange="change_rate('<?= $i;?>')" onkeyup="change_rate('<?= $i;?>')" id="qlt_<?= $i;?>" class="sq_form" placeholder="Quatity" value="<?php echo $dtl['nos'];?>"></td>
                			<td><input type="text" name="rate[]" id="rate_<?= $i;?>" class="sq_form" placeholder="Rate" readonly value="<?php echo $dtl['rate'];?>"></td>
                			<td><input type="text" name="amt[]" id="amt_<?= $i;?>" class="sq_form" onkeyup="return myKeyPress(event)" placeholder="Amount" readonly value="<?php echo $dtl['amount'];?>"></td>
                	...
                	<tfoot>
                	    <tr>
                	        ...
                	        <th style="text-align:left;padding:10px 25px" id="total"><?php echo $fpooja_list['total'];?></th>
                	        <th colspan="2"></th>
                	    </tr>
                	</tfoot>
            	</table>			
