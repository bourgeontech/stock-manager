                ...
                <div class="inner_pak">
                  <div class="row">
                    ...
                    <div class="col-lg-8 offset-lg-0 ">
                    	...
                    	<form action="<?php echo base_url(); ?>index.php/admin/membership/index" method="post">
                        <div class="row">
                          <div class="col-sm-12">
                        ...
                        if ($this->db->field_exists('plan', 'memberships')) { ?>   <?php } else {?> style="display:none;" <?php }?> >
                            <div class="form-group" id="pooja_date_group">
                              <label>Membership Plan </label>
                            	
                              &nbsp;&nbsp;<input type="radio" name="plan" <?php
                        if ($this->db->field_exists('plan', 'memberships')) { ?>   <?php } else {?> checked <?php }?> required  id="plan" value="1" >  Lifetime Membership &nbsp;<input type="radio" name="plan"  id="plan2" value="2"> Annual Membership
                            </div>
                          </div>
                      
						  <?php if ($this->db->field_exists('plan', 'memberships')) { ?>
						   <div class="col-sm-12">
                            <div class="form-group" id="sponsered">
                              <label>Category </label>
                            	
                              <select name="sponsered" id="sponsered" class="form-control" style="width:100%;">
                    ...
                    <div class="col-lg-4 shadow-sm">
                      <div class="mybooking cart">
                        <h3 class="title"><i class="fa fa-cart-arrow-down"></i> Review </h3>
                        <div class="cart_details">
                          <form action="<?php echo base_url(); ?>index.php/admin/membership/review" method="post">
                          	<input type="hidden" name="order_id" value="<?php echo $order->id; ?>" />
                          	<input type="hidden" id="total_amount" value="<?php print_r($data['total'] ?? $amount);?>" />
                          ...
                          <select name="payment_mode" id="payment_mode" class="form-control">
                            	<?php foreach($payment_modes as $mode): ?>
                            		<option value="<?php echo $mode->slug; ?>"><?php echo $mode->name; ?></option>
                            	<?php endforeach; ?>
                          	</select>
                          <div class="d-flex flex-row mt-3"> 
                          	<input type="submit" value="Save" id="save_btn" class="btn btn-success mr-2 w-100"> 
<!--                           	<input type="button" name="date" value="Pay Amount" id="make_payment_btn" data-gateway="<?= $payment_gateway ?>" class="btn btn-success mr-2 w-100">  -->
                          	<a ID="btn_discard" class="btn btn-outline-warning w-100" href="<?php echo base_url(); ?>index.php/admin/membership/discard/<?php print_r($data['billing_online_id'] ?? ''); ?>" style="text-align: center;">Discard</a>
                          </div>
                        </form>
                        </div>
                      </div>
...
<script>
	document.getElementById('mobile_no').onchange = function (e) {
    	...
    	$.ajax({
                url: "/index.php/admin/membership/getCustomerByMobileNo",
                type: "post",
                data: {
	...
	$('#payment_mode').on('change', (e) => {
    	let mode   = e.target.value;
        let amount = $('#total_amount').val()
        if(mode == 'vip' || mode == 'sponsor' || mode == 'trustee' || mode == 'staff') {
        	$('#ContentPlaceHolder1_lbltotal2').text(0)
        } else {
        	$('#ContentPlaceHolder1_lbltotal2').text(amount)
        }
    })
