...
class Membership extends CI_Controller {
	...
	public function index() {
    	...
    	if ($this->form_validation->run() === FALSE) {
        	...
        	$this->load->view('site/membership/index',$data);
            ...
            if($exists == 0) {
               if ($this->db->field_exists('gothram', 'user_dtl')) {
                $this->db->query("INSERT INTO user_dtl(`name`,`mobile`,`email`, `house`, `street`, `post`, `district`, `state`, `pincode`,`status`,'star_id','gothram') VALUES ('$name','$mobile_no', '$house', '$street', '$post', '$district', '$state', '$pincode','$email','1','$star_id','$gothram')");
            	}
				else
				{
	...
	public function payment($key){
    	...
    	if ($fetch->captured == 1) {
        		...
        		if ($query->num_rows() > 0) {
                	...
                	if ($this->db->field_exists('purpose', 'memberships')) {
					$membershipInsertData  = array(
        										'name'			=> $details[0]['name'], //$membership_data['name'], 
                	...
                	//$this->db->query("INSERT INTO billing(`diety_id`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`counter`,`bill_time`, `bill_type`, `created_at`) VALUES ('$deity','$today','$payment_id','$total','$customer_id','$count','$total', '2','$total','6','0000-00-00 00:00:00','O', '$now')");
                      if ($this->db->field_exists('purpose', 'billing')) {
                	
				$this->db->query("INSERT INTO billing(`diety_id`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`counter`,`bill_time`, `bill_type`, `created_at`,`purpose`) VALUES ('$deity','$today','$payment_id','$total','$customer_id','$count','$total', '2','$total','6','0000-00-00 00:00:00','O', '$now','$membership_data[purpose]')");
					}
					else
					{
				$this->db->query("INSERT INTO billing(`diety_id`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`counter`,`bill_time`, `bill_type`, `created_at`) VALUES ('$deity','$today','$payment_id','$total','$customer_id','$count','$total', '2','$total','6','0000-00-00 00:00:00','O', '$now')");
...
$membershipInsertData  = array(
            ...
            //$this->db->query("INSERT INTO billing(`diety_id`,`transaction_date`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`bal_amt`,`counter`,`bill_time`, `bill_type`) VALUES ('$deity','$today','$today','$key','$amd','$customer_id','$count','$total','2','$total','0','6','0000-00-00 00:00:00','O')");
				if($this->db->field_exists('purpose', 'billing')) {
            $this->db->query("INSERT INTO billing(`diety_id`,`transaction_date`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`bal_amt`,`counter`,`bill_time`, `bill_type`,'purpose') VALUES ('$deity','$today','$today','$key','$amd','$customer_id','$count','$total','2','$total','0','6','0000-00-00 00:00:00','O','$membership_data[purpose]')");
			}
			else
			{
			$this->db->query("INSERT INTO billing(`diety_id`,`transaction_date`,`date`,`place`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`bal_amt`,`counter`,`bill_time`, `bill_type`) VALUES ('$deity','$today','$today','$key','$amd','$customer_id','$count','$total','2','$total','0','6','0000-00-00 00:00:00','O')");	
...
//     	return $membership_id;
    	...
    	$prefix = 'KSMT'; // Prefix for the membership ID
	...
	public function thankyou($bill_id=null){
        ...
        $this->load->view('site/layouts/bookingheader');
        $this->load->view('site/thankyou_membership', $data);
        $this->load->view('site/layouts/admin_footer');
	...
	public function bill_print($id){
        ...
        $data['membership_data'] = $this->general_model->getMembershipById($this->session->membership_id);
    
        $this->load->library('dpdf');
        $billprint = $this->load->view('site/bill_print_membership', $data, true);
	...
	public function memberLogin() {
    	...
    	if ($this->form_validation->run() === FALSE) {
        	$this->load->view('site/layouts/bookingheader');
        	$this->load->view('site/membership/member_account');
        	$this->load->view('site/layouts/admin_footer');
        } else {
        	$mobile_number = $this->input->post('mobile_number'); 
        
        	if(!$this->isCustomerExists($mobile_number)) {
            	redirect('membership/memberLogin');
            } else {
            	$this->session->set_userdata('mobile_number', $mobile_number);
            	redirect('membership/otpForm');
            }
        }
    }
	...
	public function sendOtp($mobile, $otp){
        // $message =urlencode("Dear Customer Your verification code is $otp Regards PRNVMS");
        // $url = "http://sms.text91msg.com/http-tokenkeyapi.php?authentic-key=3930424f555247454f4e3135361653977314&senderid=PRNVMS&route=1&number=$mobile&message=$message&templateid=1607100000000317272";
	...
	public function otpForm() {
    	...
    	if ($customerExists) {
        	...
        	$this->load->view('site/layouts/bookingheader');
        	$this->load->view('site/membership/otp_form');
        	$this->load->view('site/layouts/admin_footer');
    	} else {
        	$data['message']="No membership found for the provided mobile number and membership ID.";
			$this->load->view('site/layouts/bookingheader');
        $this->load->view('site/membership/member_account',$data);
        $this->load->view('site/layouts/admin_footer');
    	}
	}
	...
	public function verifyOtp() {
        ...
        echo $otpSession = $this->session->userdata('otp'); 
    	if ($this->form_validation->run() === FALSE) {
        	$this->load->view('site/layouts/bookingheader');
        	$this->load->view('site/membership/otp_form');
        	$this->load->view('site/layouts/admin_footer');
    		...
    		if($otp != $otpSession) {
            	...
            	redirect('membership/verifyOtp');
        	} else {
            	redirect('membership/memberAccount');
            }
        }
	}
	...
	public function referFriend() {
    	...
    	if ($customer) {
        	...
        	$this->load->view('site/membership/refer_friend', $data);
    	} else {
        	redirect('customer/login');
    	}
    }
	...
	public function referByMe() {
    	...
    	if ($customer) {
        	...
        	$data['membership'] = null;
        	if ($hasMembership) {
            	$membership_details = $this->membership_model->getMembershipDetails($mobile_number,null);
        	...
        	$this->load->view('site/membership/refer_by_me', $data);
    	} else {
        	redirect('customer/login');
    	}
	...
	public function membership_report_print($membership_id){
    	...
    	$image_path = base_url() . 'assets/images/ayyappa/background.png'; }else {$image_path = base_url() . 'assets/images/ayyappa/background2.png'; }// FCPATH gives the root directory of CodeIgniter
        $resizeImage = $this->resizeImage($image_path);
    	$data['background_image'] = $this->encodeImage($resizeImage);
  //  print_r($data);exit;
		// $this->load->view('site/bill_print_membership2', $data);
    	$this->load->library('dpdf');
    	$billprint = $this->load->view('site/bill_print_membership2', $data, true);
    	$this->dpdf->createPDF($billprint, 'mypdf', false);
    	return; 
	}
...
function resizeImage($file, $maxWidth = 800, $maxHeight = 800) {
    ...
    return $tempFile; // Return path to resized image
}
	...
	public function swaswatha_pooja() {
    	...
    	if ($customer) {
        	...
        	if ($hasMembership) {
            	...
            	$this->load->view('site/membership/swaswatha_pooja', $data);
            }
	...
	public function bookings() {
    	...
    	if ($membership_details) {
        	...
        	if ($customer_id) {
            	...
            	$this->load->view('site/membership/bookings', $data);
        	} else {
