    ...
    <?php   
        ...
        foreach($deities as $deity) {
        	...
        	foreach($dates as $key => $date_arr) {
                ...
                $cat_query  = $this->db->query("SELECT pooja_cat FROM pooja WHERE id IN (SELECT pooja FROM billing_dtls WHERE bill_id = $bill_id and diety_id='$deity_id' and date='$date') GROUP BY pooja_cat");
				...
				<div style="width:100%; display:flex" id="footer">
                	<div style="text-align: left;margin-bottom:1cm;">Prepared By : <?= $preparedby->name ?? ''; ?><br><?php echo  date("g:i a", strtotime($preparedby->bill_time ?? '')); ?></div>
					<div style="margin-bottom:1cm;">CLERK <br> <?= $preparedby->countername ?? ''; ?></div>
        		</div>
				<div class="<?php echo $pageBreakClass; ?>"></div>
                <?php   
                    }
