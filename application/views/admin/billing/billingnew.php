  ...
  margin:2px;color:red;"> Bill total </span>
    ...
    <script type="text/javascript">
     ...
     jQuery( document ).ready(function( $ ) {
     	...
     	$("#dtab tbody").on('input', '.amt', function() {
             var rowindex = $(this).closest('tr').index();
             calculateRowPoojaData(rowindex);
         }); 
   ...
   function createrow(data){
         ...
         if(starcode==1){
   		 ...
   		 cols += '<input class="deity_id" value="'+data.diety+'" type="hidden" />';
       ...
       if(e.keyCode == 13) {
              ...
              if($("#dtab .name").is(':focus') == true){
                  var rowindex = $("#dtab .name").closest('tr').index();
              	  var $name = $('table.dataTable2 tbody tr:nth-child(' + (rowindex + 1) + ')').find('.name');
              	  
              	  $.ajax({
        			url: "<?php echo base_url(); ?>index.php/admin/admin/translatetext",
        	 		type: 'post',
        			dataType: 'json',
        			data: {
          				search: $name.val()
