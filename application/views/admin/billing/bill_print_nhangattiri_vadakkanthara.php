 ...
 <html>
    ...
    // $site_settings=$this->site_model->settings();
          ...
          foreach ($dates as $date){
            $da = $date['date'];
	        $b = $this->db->query("SELECT DISTINCT(pooja_cat) FROM pooja WHERE id IN (SELECT pooja FROM billing_dtls WHERE bill_id = $bill_id and date='$da') GROUP BY pooja_cat");
...
//         	    if(count($query)>0){
			...
			<table border="1" style="border-collapse:collapse;width:100%;margin-bottom:1cm;">
			    <thead>
                	...
                	<?php if($query[0]['diety_id'] == 114):
                			$customer = $this->db->where('id', $bill_list[0]['customer_id'])->get('user_dtl')->row();
                			$customer_name = $customer->name;
                			$received	   = $bill_list[0]['recv_amt'];
                			$received_in_words = $received;
                	?>
                	<tr>
    					<td colspan="9" style="padding:10px 0;">
                        	<p> Received with thanks from Sri/Smt <b> <?php echo $customer_name; ?> </b>
                            a sum of Rs. <b> <?php echo $received; ?> </b> ( Rupees <?php echo $received_in_words; ?> only)
                             for Marriage / Party/ Function conducted on <?php echo date('d-m-Y',strtotime($query[0]['date'])); ?>
    				    </td>
    				</tr>
                	<?php endif; ?>
    				<tr>
				...
				<tbody>
                	...
                	<tr>
				        ...
				        <th style="text-align:right"><?php echo $received;?></th>
				    </tr>
                	<tr>
				        <th></th>
				        <th colspan="6" style="text-align:left">Balance</th>
				        <th style="text-align:right"><?php echo $balance;?></th>
				    </tr>
                	<?php endif; ?>
				    <tr>
