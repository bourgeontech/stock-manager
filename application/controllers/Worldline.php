...
class Worldline extends CI_Controller {
    public function __construct(){
    	...
    	$sum = 0;
		$data['discount']=0;
		$data['discountype']='Fixed';
		if($this->billing_online){
        	foreach ($this->billing_online as $item) {
    			$sum += $item['total'];
				$data['discount']=0;
				$query = $this->db->query("SELECT mobile FROM user_dtl WHERE id=".$item['customer_id']."");
				$result2 = $query->first_row();
				
			@$mobile_no=$result2->mobile;
		...
		$this->discounttype = $data['discountype'];
	...
	public function booking(){
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            if($exists==0){
                ...
                $this->db->query("INSERT INTO user_dtl(`name`,`mobile`,`status`,'gothram','star_id') VALUES ('$name','$mobile_no','1','$gothram','$star_id')");
				}
				else
				{
               ...
               if ($this->db->field_exists('gothram', 'user_dtl')) {
                $this->db->query("UPDATE user_dtl SET customer_id='$customer_id',name='$name',gothram='$gothram',star_id='$star_id' WHERE mobile='$mobile_no'");
				}
				else
				{
				 $this->db->query("UPDATE user_dtl SET customer_id='$customer_id',name='$name' WHERE mobile='$mobile_no'");	
	...
	public function donation(){
       ...
       $this->form_validation->set_rules('diety_id', 'Diety', 'required');
        ...
        if ($this->form_validation->run() === FALSE){
            ...
            $this->db->where('diety_pooja.temple_id', 8);
            $query = $this->db->get();
            if ($query->num_rows() > 0) {
...
//             // $refno=$this->session->refno;
            ...
            if(!empty($dates)){
                ...
                while ($i<sizeof($dates)){
                    ...
                    $this->db->query("INSERT INTO billing_online(`customer_id`,`name`,`diety_id`,`star_id`,`pooja_id`,`rate`,`date`,`total`,`status`) VALUES ('$customer_id','$name','$deity_id','$star_id','$pooja_id','$amount','$data','$amount', '1')");
                    $bill_id=$this->db->insert_id();
					$existingData[0]['id'] = $bill_id;
					$i++;
                }
            }else{
            	$date = date('Y-m-d');
                ...
                { 
              	...
              	$this->db->query("INSERT INTO billing_online(`customer_id`,`name`,`diety_id`,`star_id`,`pooja_id`,`rate`,`date`,`total`,`status`) VALUES ('$customer_id','$name','$deity_id','$star_id','$pooja_id','$amount','$date','$amount', '1')");
			$bill_id=$this->db->insert_id();
			$existingData[0]['id'] = $bill_id;
			}
	...
	if(!$is_donation) {
    	...
    	// $total_payment= += ($total_payment*$data['temple_list'][0]['payment_charge']/100);
			$total_payment=$total_payment-$total1;
			
				}
				$bill_ids_string = implode(",", $bill_ids);
     			$this->session->set_userdata('amount', $total_payment);
    	if($paymentgateway == 1) {
    ...
    ob_start();
    	...
    	$payment_id = $key; // From your function parameter

// 1. Check if this payment ID has already been processed
$already_processed = $this->db->get_where('razorpay_transactions', ['payment_id' => $payment_id])->row();

if ($already_processed) {
    // This is a replay! Redirect to a "Transaction already processed" page.
    redirect(site_url("worldline/already_processed"));
    return;
	exit();
}
    	if ($fetch->captured == 1) {
				...
				if ($_SERVER['HTTP_HOST'] == "kaladyshankaramadomts.org" || $_SERVER['HTTP_HOST'] == "kaladyshankaramadomts.org") {
					...
					$expected_amount = $this->session->userdata('amount');
            		...
            		for($i=0;$i<count($orders);$i++){
						...
						$poojarate = $this->db->get()->result_array();
                		if($poojarate[$i]['rate']>0)
						{
						$rate=$poojarate[$i]['rate'];
						}
						else {
						$rate=$orders[0]['rate'];
						}
        		...
        		if ($query->num_rows() > 0) {
                	...
                	if($this->db->field_exists('appearance', 'billing')) {
                		...
                		$this->db->query("INSERT INTO billing(`diety_id`,`date`,`place`,`order_id`,`amount`,`customer_id`,`count`,`total`,`status`,`recv_amt`,`counter`,`bill_time`, `bill_type`, `created_at`) VALUES ('$deity','$today','$payment_id','$order_id','$totalamount','$customer_id','$count','$totalamount', '2','$totalamount','6','0000-00-00 00:00:00','O', '$now')");
                	  //echo $this->db->last_query();
                    }
...
//     }
	...
	if($_SERVER['HTTP_HOST'] == "kaladyshankaramadomts.org") {	
		$query1 = $this->db->query("select discount from billing where id = '$bill_id'");
        $discounts = $query1->result_array();
