...
if($this->db->table_exists('general_settings')) {
	$general_settings = $this->db->get('general_settings')->row();

	$needs_name_translation = $general_settings->translate_name;
} else {
	$needs_name_translation = 1;
}
?>
       ...
       <div class="row">
	     <div class="col-lg-12 col-md-12 col-sm-12 ">		 
	        ...
	        <form action="<?php echo base_url(); ?>index.php/admin/admin/billing" method="post"  id="myform">
            	...
            	<input type="hidden" name="custom_booking_id" id="custom_booking_id" value="">
            	<input type="hidden" name="advance_amount" id="advance_amount" value="">
               <div class="row">
                  <div class="col-lg-3 col-md-3 col-sm-3 ">
...
function handlePoojaDateChange(event) {
    const date = event.target.value;
    var qty = $('.qty:first').val();
    var pooja_id = $('.pooja_code:first').val();
    var promise = checkPoojaAvailability(pooja_id, date, qty, 0);
}

function handleAmount(template_id) {
    return function(event) {
        const amount = event.target.value;
        $('#advance_amount_' + template_id.toLowerCase()).attr('max', amount);
        let $row = $('#dtab tbody tr:first');
        $row.find('.amt').val(amount);
        calculateRowPoojaData($row.index());
    }
}

function handleAdvanceAmount(template_id) {
    return function(event) {
      ...
      var pooja_fetch_url = "<?php echo base_url(); ?>index.php/admin/admin/getPoojaByCodeNameDietyforbilling";
       $('#searchpooja').autocomplete({
             source: function(request, response) {
            	...
            	// $(".ui-menu-item:first").find('.ui-menu-item-wrapper').addClass('ui-state-active')
            	// }, 0);
        ...
        $( document ).ready(function() {
         ...
         $("#dtab").on('input', '.qty', function() {
     	 ...
     	 $("document").on('change', '.custom_booking_date', function(e) {
  ...
  // alert("o");
   ...
   function getCustomBookingSwal(template_id, rate) {
    	if(template_id == 'AUD') {
        	var formHtml = `
              <form id="myForm">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="date" name="date" id="custom_booking_date_aud" value="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" placeholder="Marriage Date" class="form-control custom-input custom_booking_date" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="time" name="muhoortham_time" placeholder="Muhoortham Time" class="form-control custom-input" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="text" name="address" placeholder="Billing Address" class="form-control custom-input" >
                        </div>
                    </div>
					<div class="col-md-4">
                        <div class="form-group">
                            <input type="text" name="contact_number_1" placeholder="Contact Number 1" class="form-control custom-input" required>
                        </div>
                    </div>
					<div class="col-md-4">
                        <div class="form-group">
                            <input type="text" name="contact_number_2" placeholder="Contact Number 2" class="form-control custom-input" >
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="text" name="booking_number" placeholder="Booking Number" class="form-control custom-input" >
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="number" name="amount" id="total_amount_aud"  placeholder="Total Amount" class="form-control custom-input total_amount" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="number" name="advance_amount" id="advance_amount_aud" placeholder="Advance Amount" class="form-control custom-input advance_amount" required>
                        </div>
                    </div>
                </div>
            </form>
              `;
        } else if(template_id == 'ELF') {
        	var formHtml = `
              <form id="myForm">
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="date" name="date" id="custom_booking_date_elf" value="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" placeholder="Date" class="form-control custom-input" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
				    ...
				    <div class="col-md-4">
                        <div class="form-group">
                            <input type="number" name="amount" id="total_amount_elf"  placeholder="Total Amount" class="form-control custom-input" required>
                        </div>
                    </div>
                    <div class="col-md-4">
                        <div class="form-group">
                            <input type="number" name="advance_amount" id="advance_amount_elf" placeholder="Advance Amount" class="form-control custom-input" required>
                        </div>
                    </div>
                </div>
            </form>
        ...
        $.ajax({
           ...
           success: function(data) {
                ...
                if(diety == '102' || diety == '103') {
           		...
           		let is_custom_bookings = data.custom_booking ? true : false;
           		if(is_custom_bookings) {
                	let template = getCustomBookingSwal(data.template, data.rate)
                	Swal.fire({
              ...
              preConfirm: () => {
                ...
                let pooja_date = formData.get('date')
                var qty 	 = $('.qty:first').val();
        		var pooja_id = $('.pooja_code:first').val();
                var promise = checkPoojaAvailability(pooja_id, pooja_date, qty, 0);
        		promises.push(promise);

				var is_validated = form.checkValidity();
   				Promise.all(promises)
            		.then(results => {
              			var flag = results.every(result => result === true);
            		})
                ...
                if (is_validated) {
                    $('.pdate').val(pooja_date);
                
					formData.append('pooja_id', id);
					formData.append('type', data.template);
                	
                
                	let total   = formData.get('amount')
                    let advance = formData.get('advance_amount')
                    ...
                    $.ajax({
                        ...
                        type: "POST",
                        url: '<?php echo base_url(); ?>index.php/admin/custombooking/update_temp_data' ,
                        data: formData,
                    	processData: false, 
    					contentType: false,
                        success: function (response) {
                            if (response && response.status == 'success') {
                                let booking_id = response.custom_booking_id
                                
                                $('#custom_booking_id').val(booking_id)
                            }
                        }
                    });
                  ...
                  var customValidationMsg = ''
                  invalidInputs.forEach(input => {
                    $(input).css('border-color', 'red');
                  	customValidationMsg = input.validationMessage
                  });
                  
                  if(customValidationMsg) {
                  	Swal.showValidationMessage(customValidationMsg)
                  } else {
              ...
              didOpen: () => {
                const dateInputAud = document.getElementById('custom_booking_date_aud');
        		if (dateInputAud) dateInputAud.addEventListener('change', handlePoojaDateChange);

        		const dateInputElf = document.getElementById('custom_booking_date_elf');
        		if (dateInputElf) dateInputElf.addEventListener('change', handlePoojaDateChange);

        		const totalAmountAud = document.getElementById('total_amount_aud');
        		if (totalAmountAud) totalAmountAud.addEventListener('input', handleAmount('AUD'));

        		const totalAmountElf = document.getElementById('total_amount_elf');
        		if (totalAmountElf) totalAmountElf.addEventListener('input', handleAmount('ELF'));

        		const advanceAmountAud = document.getElementById('advance_amount_aud');
        		if (advanceAmountAud) advanceAmountAud.addEventListener('input', handleAdvanceAmount('AUD'));

        		const advanceAmountElf = document.getElementById('advance_amount_elf');
        		if (advanceAmountElf) advanceAmountElf.addEventListener('input', handleAdvanceAmount('ELF'));
//                 
  ...
  function createrow(data)
  {
    ...
    createsecondrow(data);
         ...
         cols += '<td><input class="sq_form rate w-100" name="rate[]" value="' + data.rate + '"  style="width: 60px;" required  '+rate_readonly+'/></td>';
         cols += '<td><input  class="sq_form amt w-100" name="amt[]" value="0.00" type="text" style="width: 70px;" required '+toggleamtreadonly+'/></td>';
...
//             }
  ...
  <?php if($needs_name_translation == 1) { ?>
 ...
 <?php } ?>
   ...
   <?php } ?>
    <?php
    if($_SERVER['HTTP_HOST'] == "vasai.templesoftware.in")
	{
    ?>
  $(document).on("keydown", function(e) {
    if (e.key === "Enter") {
        const $focused = $(':focus');
        if ($focused.hasClass('qty')) {
            e.preventDefault();
            const $row = $focused.closest('tr');
            $row.find('.amt').focus().select();
        }
    }
});
    
    $(document).on("keydown", function(e) {
    if (e.key === "Enter" || e.keyCode === 9) { // Tab = 9
        const $focused = $(':focus');
        if ($focused.hasClass('qty')) {
            e.preventDefault();
            const $row = $focused.closest('tr');
