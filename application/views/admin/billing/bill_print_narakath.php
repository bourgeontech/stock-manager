...
//         	    if(count($query)>0){
		    ?>
			<table border="1" style="border-collapse:collapse;width:100%;margin-bottom:1em;" class="table table-bordered">
			    <thead>
    				<tr>
    					<td colspan="9" style="width:100%;">
                        <div class="d-flex flex-row justify-content-between">
    						<img alt="Image" src="<?php echo $site_settings['bill_image'];?>" style="height: 100px;width: auto;" />
    						<h4 style="text-align:center; font-size:15px;">
                            	<?php print_r($temple_list[0]['name']);?><br>
    							<span style="text-align:center;font-size:11px;"><?php print_r($temple_list[0]['address']." <br>".$temple_list[0]['location']." ".$temple_list[0]['phone']);?></span>
                            </h4>
                        	<?php 
                			$emblem_right = $site_settings['emblem_right'] ?? null;
                			if($emblem_right):
                			?>
                			<img alt="Image" src="<?php echo $emblem_right;?>" style="height: 100px;width: auto;" />
                        	<?php else: ?>
...
<!--                             	<label style="float:center;text-align:center;"><?php print_r($query[0]['deity_nm']);?></label> -->
                            	<label style="float:right;font-size:bold;">Bill - <?php print_r($bill_list[0]['bill_no'] ?? $query[0]['bill_id']);?></label>
                        	</div>
    				    </th>
    				</tr>
				    ...
				    foreach($query as $val){ 
                       ...
                       ?>
				        <tr style="text-align:center;">
				            ...
				            <td style="padding: 0px 5px;"><b><?php echo $val['pooja_nm']; if ($date!=$today){echo "(".date('d-m-Y',strtotime($date)).")";}?></b></td>
<!-- 				            <td style="text-align:right"><?php  echo $val['time'];?></td>   
				...
				<?php if($temple_list[0]['website']!='') { ?>
                	<tr>
				        <th colspan="6" class="bottom-content" >For Online Pooja booking  <a href="<?php print_r($temple_list[0]['website']);?>"><?php print_r($temple_list[0]['website']);?></a>   - </th>
				    </tr>
                <?php } ?>
				</tbody>
			</table>
        	...
        	<div style="width:100%;	 display:flex; justify-content: space-between;">
            	...
            	<?php endif; ?>
        		<strong style="text-align: right; margin-top:auto" class="bottom-content">Payment Method:  <?= $paymentMode ?? ''; ?></strong>
        	</div>

        	<div style="width:100%; display:flex; margin-top:5px;">
              <strong style="flex:1; text-align: left;margin-bottom:1cm;float:left;" class="bottom-content">
<!--               	Prepared By : <?= $preparedby->name ?? ''; ?><br> -->
              	<?php echo  date("d/m/Y g:i a", strtotime($preparedby->bill_time ?? '')); ?><br></strong>
				<strong style="flex:1; text-align: right;margin-bottom:1cm;float:right;" class="bottom-content">CLERK <br> <?= $preparedby->countername ?? ''; ?></strong>
