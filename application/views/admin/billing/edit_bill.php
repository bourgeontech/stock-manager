   ...
   <!--     </div>-->
	  ...
	  <!--</div>-->
       <div class="row">
		 ...
		 $bill = $this->db->where('id', $bill_id)->get('billing')->row();
	$created_time = strtotime(@$bill->bill_time);
    $now_time     = time();
    $diff_minutes = ($now_time - $created_time) / 60;
 if (!in_array($this->loggedIn['role'], ['admin', 'superadmin'])) 
    {
    // if more than 10 mins and not same user → block
 
	if ($bill->user_id != $this->loggedIn['id']) {
        $this->session->set_flashdata('error', 'You cannot edit this bill.');
...
if (!in_array($this->loggedIn['role'], ['admin', 'superadmin'])) 
  ...
  if ($diff_minutes < 10 ) {
        ?>
         	...
         	<form action="<?php echo base_url(); ?>index.php/admin/admin/edit_bill/<?php echo $bill_id;?>" method="post" onsubmit="return validateForm()" id="myform">
         	<?php if($customer): ?>
         	<div class="card mb-3 p-3">
            	<div class="form-group">
                  <div class="row_form">
                  		<h5><i class="fa fa-user-o" aria-hidden="true"></i>  Customer Details</h5>
        				<table class="table table-responsive-sm">
                        	...
                        	<tbody>
                				<tr>
                                	<td> <input type="hidden" class="form-control" name="customer_details[id]" value="<?php echo $customer[0]['id']; ?>" /> <input type="text" class="form-control" name="customer_details[name]" value="<?php echo $customer[0]['name']; ?>" /> </td>
                					<td> 
                                    	<input type="text" class="form-control" name="customer_details[mobile]" value="<?php echo $customer[0]['mobile']; ?>" /> 
                                    	<?php if($this->session->flashdata('mobile_error')) { ?>
                                    	<small class="text-danger"> <?php echo $this->session->flashdata('mobile_error'); ?> </small>
                                    	<?php } ?>
                                	</td>
                					<td> <input type="email" class="form-control" name="customer_details[email]" value="<?php echo $customer[0]['email']; ?>" /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[house]" value="<?php echo $customer[0]['house']; ?>" /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[post]" value="<?php echo $customer[0]['post']; ?>" /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[street]" value="<?php echo $customer[0]['street']; ?>" /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[district]" value="<?php echo $customer[0]['district']; ?>" /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[state]" value="<?php echo $customer[0]['state']; ?>" /> </td>
                					<td> <input type="number" class="form-control" name="customer_details[pincode]" value="<?php echo $customer[0]['pincode']; ?>" /> </td>
                            	</tr>
                			</tbody>
        				</table>
                  </div>
      ...
      <div class="form_body">
        <div class="row">
		   ...
		   <div class="col-lg-12">
			  ...
			  {?>
							<td>
							<input type="text" name="gothram[]" id="gothram_100" class="sq_form gothram" placeholder="Gothram" value="<?php echo $dtls['gothram'];?>">
                			</td>
                           <?php } ?>
                            ...
                            <input type="hidden" name="bt[]" value="<?php echo $dtls['id'];?>">
                			...
                			<td><input type="text" name="rate[]" id="rate_<?= $i;?>" class="sq_form" placeholder="Rate" readonly value="<?php echo $dtls['rate'];?>"></td>
                			...
                			<td><input type="date" name="date1[]" id="date_<?= $i;?>" class="sq_form datefield" value="<?php echo $dtls['date'];?>"  onchange="checkallowedqty('<?= $i;?>');" onclick="checkallowedqty('<?= $i;?>');"  style="max-width: 4.2cm;"></td>
                		    <td><select name="time[]" id="time_<?= $i;?>"  onkeypress="return myKeyPress(event)"  class="sq_form" required style="width: 1.6cm;">
                    				<option value="M" <?php if($dtls['time']=="M"){ echo "selected";}?>>M</option>
                    				<option value="E" <?php if($dtls['time']=="E"){ echo "selected";}?>>E</option>
                            <option value="N" <?php if($dtls['time']=="N"){ echo "selected";}?>>N</option>
                            <option value="8" <?php if($dtls['time']=="8"){ echo "selected";}?>>8</option>
                    			</select>
                			</td>	
            ...
            <div class="form-group" style="margin-top: 7px;" id="paymenttable">
                  <select id="mode" name="mode" class="sq_form" required onchange="openpaydetails()" >
                          <?php foreach($payment_modes as $mode): ?>
                 		  <option value="<?php echo $mode['id']; ?>" <?php if($mode['id'] == $bill_list[0]['mode']) echo 'selected'; ?>><?php echo $mode['name']; ?></option>
                 		  <?php endforeach; ?>
                  </select>
               </div>
            </div>
  ...
  <form action="<?php echo base_url(); ?>index.php/admin/admin/edit_bill/<?php echo $bill_id;?>" method="post" onsubmit="return validateForm()" id="myform">
        				...
        				<table class="table table-responsive-sm">
                        	...
                        	<tbody>
                				<tr>
                                	<td> <input type="hidden" class="form-control" name="customer_details[id]" value="<?php echo $customer[0]['id']; ?>" /> <input type="text" class="form-control" name="customer_details[name]" value="<?php echo $customer[0]['name']; ?>" /> </td>
                					<td> 
                                    	<input type="text" class="form-control" name="customer_details[mobile]" value="<?php echo $customer[0]['mobile']; ?>" readonly /> 
                                    	<?php if($this->session->flashdata('mobile_error')) { ?>
                                    	<small class="text-danger"> <?php echo $this->session->flashdata('mobile_error'); ?> </small>
                                    	<?php } ?>
                                	</td>
                					<td> <input type="email" class="form-control" name="customer_details[email]" value="<?php echo $customer[0]['email']; ?>" readonly /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[house]" value="<?php echo $customer[0]['house']; ?>" readonly /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[post]" value="<?php echo $customer[0]['post']; ?>" readonly /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[street]" value="<?php echo $customer[0]['street']; ?>" readonly /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[district]" value="<?php echo $customer[0]['district'];  ?>" readonly /> </td>
                					<td> <input type="text" class="form-control" name="customer_details[state]" value="<?php echo $customer[0]['state']; ?>" readonly /> </td>
                					<td> <input type="number" class="form-control" name="customer_details[pincode]" value="<?php echo $customer[0]['pincode']; ?>" readonly /> </td>
                            	</tr>
                			</tbody>
        				</table>
                  </div>
      ...
      <div class="form_body">
        <div class="row">
		   ...
		   <div class="col-lg-12">
			  ...
			  {?>
                    ...
                    if(!empty($bill_dtls)){
                		...
                		<tr>
                			...
                			<td><input type="text" name="name[]" id="name_<?= $i;?>" class="sq_form" placeholder="Name" value="<?php echo $dtls['name'];?>" required style="width: 3.2cm;" readonly></td>
                			<td><select name="star[]" id="star_<?= $i;?>" class=" sq_form" required style="width: 3.2cm;"  readonly>
			  ...
			  {?>
							<td>
							<input type="text" name="gothram[]" id="gothram_100" class="sq_form gothram" placeholder="Gothram" value="<?php echo $dtls['gothram'];?>" readonly>
                			</td>
                           <?php } ?>
							<td><select name="pooja[]" id="pooja_<?= $i;?>" onchange="change_rate('<?= $i;?>')" onload="change_rate('<?= $i;?>')" class="sq_form" required style="width: 10cm;" disabled>
                                	<option value="">Select Pooja</option>
                			...
                			<td><input type="hidden" value="<?= $i;?>" id="row_value">
                            <input type="hidden" name="bt[]" readonly value="<?php echo $dtls['id'];?>">
                			    <input type="number" min="1" readonly name="qlt[]" onchange="change_rate('<?= $i;?>'),checkallowedqty('<?= $i;?>')"  onkeyup="change_rate('<?= $i;?>'),checkallowedqty('<?= $i;?>')" id="qlt_<?= $i;?>" class="sq_form" placeholder="Quatity" value="<?php echo $dtls['qlt'];?>" disabled></td>
                			<td><input type="text" name="rate[]" readonly readonly id="rate_<?= $i;?>" class="sq_form" placeholder="Rate" readonly value="<?php echo $dtls['rate'];?>"></td>
                			<td><input type="text" name="amt[]" readonly id="amt_<?= $i;?>" class="sq_form" onkeyup="totalcalc()" placeholder="Amount" readonly value="<?php echo $dtls['amount'];?>"></td>
                			<td><input type="date" name="date1[]"  id="date_<?= $i;?>" class="sq_form datefield" value="<?php echo $dtls['date'];?>"  onchange="checkallowedqty('<?= $i;?>');" onclick="checkallowedqty('<?= $i;?>');"  style="max-width: 4.2cm;"></td>
                		    <td><select name="time[]" id="time_<?= $i;?>"   onkeypress="return myKeyPress(event)"  class="sq_form" required style="width: 1.6cm;">
                    				<option value="M" <?php if($dtls['time']=="M"){ echo "selected";}?>>M</option>
                    				<option value="E" <?php if($dtls['time']=="E"){ echo "selected";}?>>E</option>
    ...
    // 
     $('.datefield').each(function(){
    	 $(this).val($.datepicker.formatDate('dd-M-y', $(this).val());
     });
