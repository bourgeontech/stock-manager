...
<body onload="printcontend('printer')" onafterprint="myFunction()">
    <?php 
		...
		$bill_no    = $bill_id;
		$grand_total= $this->db->select_sum('amount', 'total')->from('billing_dtls')->where('bill_id', $bill_id)->get()->row()->total;
	?>
	...
	<div id="printer">
        ...
        <table border="1" style="border-collapse:collapse;width:100%;font-family:Arial;font-size:18px;padding:1 px;border 1px solid color #CCC;">
            <thead>
            	<tr>
                	<th colspan="2" style="border-right:none; text-align:left !important;"><?php echo $bill_date ?? ''; ?></th>
                	<th colspan="2" style="border:none; text-align:center !important;" class="deity"><?php echo ''; ?></th>
            ...
            <tbody>
                ...
                <?php 
                	  $j++;
					  $pageBreakClass = $count == $j ? '' : 'page-break';
                      ...
                      $sub_total = 0;
                 foreach($detail as $key => $pooja_detail): 
                	  $k++; 
                	  $pageBreak1	   = $k%7  == 0 ? 1 : 0;
                	  $pageBreak1Class = $pageBreak1  == 1 ? 'page-break' : '1';
                	  
                      $total += $pooja_detail['amount'];
                	
                	  $sub_total += $pooja_detail['amount'];
                ?>
                <tr style="text-align:center;font-size:18px;" >
                    <td><?php echo $k; ?></td>
                	<td  style="width: 50px;"><?php echo $pooja_detail['name'];?></td>  
                	<td><?php echo $pooja_detail['star_name'];?></td> 
                	<td><?php echo $pooja_detail['deity_name'];?></td>  
                    <td><?php echo $pooja_detail['pooja']; if ($pooja_detail['date']!=$today){echo "(".date('d-m-Y',strtotime($pooja_detail['date'])).")";}?></td>
            	...
            	<?php if($pageBreak1 == 1 && $k < $detail_count): $sub_total = 0; ?>
            	<tr class="<?php echo $pageBreak1Class; ?>">
                    <th></th>
                    <th colspan="7" style="text-align:left">Sub Total</th>
...
<!--         	<div class="total">
	...
	<script>
    	...
    	function printcontend(value) {
        	const elm = document.querySelectorAll('.break')
