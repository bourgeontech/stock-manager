                			...
                			<table class="table table-hover srp_table" width="100%" border="1" id="table1">
                		    	...
                		    	<!--	<tr>
                	    			...
                	    			<tr>
                		    			...
                		    			<td style="text-align: right;"><?php echo (number_format($close_bank, 2, '.', ''))?></td>
                	    			</tr>-->
                	    			<?php 
                	    			$i=0;
                	    			$c_tot=0;
                	    			...
                	    			foreach ($query2 as $val){
                	    			    ...
                	    			    $sdf = $this->db->query("SELECT sum(amount) as receipt FROM `payment` WHERE `payment_date` <= '$convertDate' and `type`='2' and ledger='$led_id' and is_delete=0")->row_array();
                	    			    $amd=$sdf['receipt'];
                	    			    $sdf1 = $this->db->query("SELECT sum(amount) as receipt FROM `payment` WHERE `payment_date` <= '$convertDate1' and `type`='2' and ledger='$led_id' and is_delete=0")->row_array();
                	    			    $amdtot=$sdf1['receipt'];
                	    			    $c_tot=$c_tot+$amd;
                	    			    $b_tot=$b_tot+$amdtot;
                	    			    if ($amd!=""||$amdtot!="") {
                			...
                			<table class="table table-hover srp_table" width="100%" border="1" id="table2">
                		    	...
                		    	<tbody>
                	    			...
                	    			foreach ($query2 as $val){
                	    			    $led_id=$val['led_Id'];
                	    			    $led_mal=$val['name_mal'];
                	    			    $sdf = $this->db->query("SELECT sum(amount) as receipt FROM `payment` WHERE `payment_date` <= '$convertDate' and `type`='1' and ledger='$led_id' and is_delete=0")->row_array();
                	    			    $amd=$sdf['receipt'];
                	    			    $sdf1 = $this->db->query("SELECT sum(amount) as receipt FROM `payment` WHERE `payment_date` <= '$convertDate1' and `type`='1' and ledger='$led_id' and is_delete=0")->row_array();
                	    			    $amdtot=$sdf1['receipt'];
                	    			    $c_tot1=$c_tot1+$amd;
                	    			...
                	    			<tr>
                	    				<td>കയ്യിരുപ്പ്</td>
                	    				<td style="text-align: right;"><?php echo (number_format($closing_cash, 2, '.', ''));?></td>
                	    				<td style="text-align: right;"><?php echo (number_format($closing_cash, 2, '.', ''));?></td>
                	    			</tr>
                	    			<tr>
                		    			<td>ബാങ്ക് ബാലൻസ്</td>
