<html>
	<head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Karla:wght@300&family=Oswald&display=swap" rel="stylesheet">
    ...
    <?php   
        ...
        $deity_query = $this->db->query("SELECT diety_id FROM billing_dtls WHERE bill_id = $bill_id GROUP BY diety_id");
		$deities=$deity_query->result_array();
        
        foreach ($deities as $deity) {
            $deity_id = $deity['diety_id'];
          
          	$dates_query = $this->db->query("SELECT date FROM billing_dtls WHERE bill_id = $bill_id AND diety_id=$deity_id GROUP BY date ");
		    $dates=$dates_query->result_array();
          
          	foreach ($dates as $date1) {
            	$date = $date1['date'];
            
	        	$pooja_cat_query = $this->db->query("SELECT pooja_cat FROM pooja WHERE id IN (SELECT pooja FROM billing_dtls WHERE bill_id = $bill_id and diety_id='$deity_id' AND date='$date') GROUP BY pooja_cat");
            	$pooja_cat		 = $pooja_cat_query->result_array();
          		$donation_print  = $site_settings['donation_print'] ?? 0;
			
          		if ($donation_print == 1 && $da == 8) {
            	...
            	foreach ($pooja_cat as $id){
                    ...
                    if(sizeof($query)>0) {    ?>
                    <div id="printer" class="p-2">
                        <table border="1" style="border-collapse:collapse;width:100%;margin-bottom:1cm;" class="table table-bordered">
                            <thead>
                                <tr>
                                    <td colspan="9" style="width:100%;">
                                    <div class="d-flex flex-row justify-content-between">
                                        <img alt="Image" src="<?php echo $site_settings['bill_image'];?>" style="height: 1.7cm;width: auto;" />
                                        <h4 style="text-align:center; font-size:15px; font-weight:bolder;">
                                            <?php print_r($temple_list[0]['name']);?><br>
                                            <span style="text-align:center;font-size:13px;font-weight:bolder;"><?php print_r($temple_list[0]['address']." <br>".$temple_list[0]['location'].",".$temple_list[0]['phone']);?></span>
                                        </h4>
                                        <?php 
                                        $emblem_right = $site_settings['emblem_right'] ?? null;
                                        if($emblem_right):
                                        ?>
                                        <img alt="Image" src="<?php echo $emblem_right;?>" style="height: 1.7cm;width: auto;" />
                                        <?php else: ?>
                                        <div style="height: 1.7cm;width: auto;"></div>
                                        <?php endif; ?>
                                    </div>
                                    </td>
                                </tr>
                                <tr>
                                    <th colspan="9">
                                        <div class="d-flex flex-row justify-content-between w-100"> <?php if(@$bill_list[0]['bill_no']) @$billno = $bill_list[0]['bill_no']; else $billno = $query[0]['bill_id']; ?>
                                            ...
                                            <label style="float:left;font-size:bold;">Bill - <?php print_r($billno);?></label>
                                        </div>
                                    </th>
                                </tr>
                            ...
                            <tbody>
                                ...
                                <tr style="text-align:center;">
                                    <td><?php echo $i;?></td>
                                    <td><b><?php echo strtoupper($val['name']);?></b></td>
                                    <td><b><?php echo $val['star_eng'];?></b></td>
                                    <td><b><?php echo $val['pooja_nm']; if ($date!=$today){echo "(".date('d-m-Y',strtotime($date)).")";}?></b><?php if($val['pooja']=="9040" || $val['pooja']=="9041" || $val['pooja']=="9042" ){  ?><p>നിവേദ്യങ്ങൾക്കായി ഒരു പാത്രം( അര ലിറ്റർ കൊള്ളുന്നത് ) ഒരു സഞ്ചി, ഈ രസിതിയും  കൂടി 9 മണിക്ക് മുന്നേ ക്ഷേത്രത്തിൽ എത്തിച്ചേരുക.</p> <?php } ?></td>
                        ...
                        <?php if($pageBreak == 1): ?>
                        <h4>Grand Total: <span style=""> <?= $grand_total ?? ''; ?> </span> </h4>
                        <?php endif; ?>

                        <div style="width:100%;	height:50px; display:flex; justify-content: space-between;">
                            <?php if($token): ?>
                            <strong style="text-align: left;margin-bottom:1cm;font-size:26px;">Token: <span style=""> <?= $token ?? ''; ?> </span> </strong>
                            <?php endif; ?>
                            <strong style="text-align: right;margin-bottom:1cm; ">Payment Method:  <?= $paymentMode ?? ''; ?></strong>
                        </div>
