	...
	<body onload="printcontend('printer')" onafterprint="myFunction()">
		<div id="printer">
		   ...
		   // $diety_id=$this->db->query("SELECT id From diety")->get()->result_array();
		        $this->db->select('billing_dtls.*,stars.name_mal as star_eng,pooja.name_mal as pooja_nm,billing_dtls.rate as pooja_rt,billing_dtls.qlt as qlt , billing_dtls.postal_amt as postalamt,billing_dtls.amount as prasad_rt,diety.name_mal as deity_nm,billing.count,billing.customer_id,billing.total,billing.recv_amt');
...
<!--     				<tr>
				...
				<tbody>
                	...
                	<?php if($pageBreak == 1 && ($key+1) < $count):  ?>
            		<tr class="<?php echo $pageBreakClass; ?>">
                    	<th></th>
                    	<th colspan="6" style="text-align:left">Sub Total</th>
                    	<th style="text-align:right"><?php echo $sub_total;?>/-</th>
               	 	</tr>
            		<?php $sub_total = 0; endif; ?>
<!--                 	<tr class="">  </tr> -->
				 <?php } ?>
                    <tr >
                    	<th></th>
