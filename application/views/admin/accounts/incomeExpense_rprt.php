        ...
        <div class="col-lg-8 col-md-8 col-sm-8">
          ...
          <input type="date" class="form-control" name="datet" value="<?php if (isset($datet)&&$datet!=""){echo $datet;}else{ echo date('Y-m-d');}?>" required aria-label="Search Attendance By Day" aria-describedby="basic-addon2">
                      <div class="input-group-append">
                        ...
                        <button class="btn btn-outline-secondary" type="button" name="search" onclick="printcontend('printer')"  value="print"><i class="fa fa-print" aria-hidden="true"></i></button>
                      </div>
                    </div>
            </form>
        </div>
            	 ...
            	 <div class="row" >
                	...
                	<h4 style="text-align:center;"><?php print_r($temple_list[0]['name']);?><br>
                        ...
                        <td>
                            ...
                            $netexpense = $exptot + $close_cash + $closing_bank;
                	    			$i=0;
                	    			$c_tot=0;
                	    			$b_tot=0;
                            $ledtotr=0;
                            ...
                            $income_over_expense = 0;
$expense_over_income = 0;

if ($grosscash > $netexpense) {
    $income_over_expense = $grosscash - $netexpense;
}
                	    			?>
                            ...
                            <tr>
                		    			<td></td>
                		    			<td>Total Income</td>
                                    <td style="text-align: right;"><?php echo (number_format($ledtotr, 2, '.', ''))?></td>
                		    	<!--		<td style="text-align: right;"><?php echo (number_format($open_bank, 2, '.', ''));?></td>
                		    	-->		
                	    			</tr>
                            <tr>
                           	...
                           	<?php
if($this->db->field_exists('incomeexpensereport', 'site_settings')){									
									$open_banknew=array();
									foreach($bankledgers as $val){
										  $pay2 = $this->db->query("SELECT sum(payment.amount) as payment,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE `group`='3' AND payment.is_delete!=1 AND `payment_date` < '$convertDate' and `type`='1' and ledger.led_Id='$val[led_Id]'")->row_array();
        $pay3=$pay2['payment'];
		$rec2 = $this->db->query("SELECT sum(payment.amount) as receipt,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE `group`='3' AND payment.is_delete!=1 AND `payment_date` < '$convertDate' and `type`='2' and ledger.led_Id='$val[led_Id]'")->row_array();
        $rec3=$rec2['receipt'];
        @$open_banknew[$val['led_Id']]=$rec3-$pay3;
										?>
									<tr>
                		    			<td></td>
                		    			<td> 
										...
										<?php echo $this->lang->line('bank_balance'); ?> (<?php echo $val['name'];?>) </td>
                                    <td style="text-align: right;"><?php echo (number_format($open_banknew[$val['led_Id']], 2, '.', ''))?></td>
...
<?php } } ?>
                                ...
                                if ($netexpense > $grosscash) {
    $expense_over_income = $netexpense - $grosscash;
}
                                ?>
<?php if ($expense_over_income > 0): ?>
<tr>
<td></td>
    <td>Expenditure over Income</td>
    <td style="text-align:right;">
        <?php echo number_format($expense_over_income, 2, '.', ''); ?>
    </td>
</tr>
<?php endif; ?>
                ...
                <td>
                			...
                			<table width="100%" border="1" id="table2">
                              ...
                              //  $this->db->where('ledger.incomeexpense', 1);
                                ...
                                <tr>
                	    				<td>Total Expense</td>
                	    				<td style="text-align: right;"><?php echo (number_format($exptot, 2, '.', ''));?></td>
                                ...
                                <?php 
									...
									if($this->db->field_exists('incomeexpensereport', 'site_settings')){
									foreach($bankledgers as $val){
										  $close_bank_pay = $this->db->query("SELECT sum(payment.amount) as payment,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE `group`='3' AND payment.is_delete!=1 AND  `payment_date` between '$convertDate' and '$convertDate1' and `type`='1' and ledger.led_Id='$val[led_Id]'")->row_array();
        $pay5=$close_bank_pay['payment'];
		$close_bank_rec = $this->db->query("SELECT sum(payment.amount) as receipt,ledger.* FROM `payment` Join ledger On payment.mode=ledger.led_Id WHERE `group`='3' AND payment.is_delete!=1 AND  `payment_date` between '$convertDate' and '$convertDate1' and `type`='2' and ledger.led_Id='$val[led_Id]'")->row_array();
        $rec5=$close_bank_rec['receipt'];
        $closing_banknew=($open_banknew[$val['led_Id']]+$rec5)-$pay5;
										?>
									<tr>
										...
										<?php echo $this->lang->line('bank_balance'); ?> (<?php echo $val['name'];?>) </td>
                                    <td style="text-align: right;"><?php echo (number_format($closing_banknew, 2, '.', ''))?></td>
                            		...
                            		<tr>
                	    				...
                	    				<td style="text-align: right;"><?php echo (number_format($exptot+$close_cash+$closing_bank, 2, '.', ''));?></td>
                	    				<!--<td style="text-align: right;"><?php echo (number_format($b_tot, 2, '.', ''));?></td>
                	    	-->		</tr>
                                ...
                                <?php
                                //if($grosscash>$netexpense){$net=$grosscash-$netexpense;
                                
                                
                                ?>    
                               <?php if ($income_over_expense > 0): ?>
<tr>
    
    <td>Income over Expenditure</td>
    <td style="text-align:right;">
        <?php echo number_format($income_over_expense, 2, '.', ''); ?>
    </td>
</tr>
<?php endif; ?>
                                ...
                                <tr>
                	    				<td>ആകെ</td>
                	    				<td style="text-align: right;"><?php echo (number_format($netexpense+$net, 2, '.', ''));?></td>
                	    				<!--<td style="text-align: right;"><?php echo (number_format($b_tot, 2, '.', ''));?></td>
                	    	-->		</tr>
                		    	</tbody>
                			</table>
