                ...
                if ((int)$pooja != 0) {
                    ...
                    if(!empty($dates)&&$pooja!=0&&$pooja!=""){
                        ...
                        while ($i<sizeof($dates)){
							...
							if($rowcount==1)
							{
								$this->db->query("INSERT INTO billing_online(`customer_id`, `reference_no`, `name`, `name_locale`,`diety_id`,`star_id`,`pooja_id`,`qty`,`rate`,`time`,`date`,`status`,`count`,`total`,`prasadam`, `prasadam_amt`) VALUES ('$customer_id', '$refno','$name[0]','$name_locale[0]','$diety_id','$star_id[0]','$pooja','$qty[0]','$rate','$time','$data', '1','$count','$pcharge','$has_prasadam', '$postel')");
							}
							else {
