...
foreach($name as $key => $nam)
			{
                            $this->db->query("INSERT INTO billing_online(`customer_id`, `reference_no`, `name`, `name_locale`,`diety_id`,`star_id`,`pooja_id`,`qty`,`rate`,`time`,`date`,`status`,`count`,`total`,`prasadam`, `prasadam_amt`) VALUES ('$customer_id', '$refno','$nam','$name_locale[$key]','$diety_id','$star_id[$key]','$pooja','$qty[$key]','$rate','$time','$data', '1','$count','$pcharge','$has_prasadam', '$postel')");
			}
						...
						foreach($name as $key => $nam)
			{
                        $this->db->query("INSERT INTO billing_online(`customer_id`,`reference_no`, `name`,`name_locale`,`diety_id`,`star_id`,`pooja_id`,`qty`,`rate`,`time`,`date`,`status`,`count`,`total`,`prasadam`, `prasadam_amt`) VALUES ('$customer_id', '$refno','$nam','$name_locale[$key]','$diety_id','$star_id[$key]','$pooja', '$qty[$key]', '$rate','$time','$main_date', '1','$count','$pcharge','$prasadam_yesno', '$postel')");
			}
    ...
    if ((int)$pooja != 0) {
        ...
        if ($rowcount == 1) {
            // ✅ Insert only once
            $this->db->query("INSERT INTO billing_online 
                (`customer_id`, `reference_no`, `name`, `name_locale`,`diety_id`,`star_id`,
                 `pooja_id`,`qty`,`rate`,`time`,`date`,`status`,`count`,`total`,
                 `prasadam`, `prasadam_amt`)
                VALUES (
                 '$customer_id', '$refno', '".$name[0]."', '".$name_locale[0]."', 
                 '$diety_id', '".$star_id[0]."', '$pooja', '".$qty[0]."', 
                 '$rate', '$time', '$main_date', '1', '$count', '$pcharge', 
                 '$prasadam_yesno', '$postel'
            )");
        } else {
            // ✅ Your old logic remains for multiple rows
            if (!empty($dates) && $pooja != 0 && $pooja != "") {
                $i = 0;
                while ($i < sizeof($dates)) {
                    $data = $dates[$i];
                    ...
                    foreach ($name as $key => $nam) {
                        $this->db->query("INSERT INTO billing_online
                            (`customer_id`, `reference_no`, `name`, `name_locale`,`diety_id`,`star_id`,
                             ...
                             `prasadam`, `prasadam_amt`)
                            VALUES (
                             '$customer_id', '$refno', '$nam', '".$name_locale[$key]."', 
                             '$diety_id', '".$star_id[$key]."', '$pooja', '".$qty[$key]."', 
                             '$rate', '$time', '$data', '1', '$count', '$pcharge', 
                             '$has_prasadam', '$postel'
                        )");
                    }
                    $i++;
