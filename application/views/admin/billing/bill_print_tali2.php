		  ...
		  $site_settings=$this->site_model->settings();
		   $a=$this->db->query("SELECT id From diety");
		    ...
		    foreach ($diety_id as $id){
		        ...
		        $id=$id['id'];
		        $this->db->select('billing_dtls.*,billing.status as bstatus,billing.total as btotal,billing.customer_id ,stars.name_eng as star_eng,pooja.name_mal as pooja_nm, pooja.name as pooja,billing_dtls.rate as pooja_rt,diety.name as deity,diety.name_mal as deity_nm');
        	...
        	<?php 
                if($bill_list[0]['status'] == 2) { 
            		$bill_date = $bill_list[0]['transaction_date'];
                } else {
			    ...
			    <thead>
    				...
    				<tr>
    					<th colspan="9"><label style="float:left;"><?php print_r(date('d-m-Y',strtotime($bill_date)));?></label>
                        	<label style="float:center;text-align:center;"><?php print_r($query[0]['deity']);?></label>
                     ...
                     $amt=$val['amount'];
                      ...
                      $amount=$val['amount'];
				        ...
				        <tr style="text-align:center;">
				            ...
				            <td><?php echo $val['pooja']; if ($date!=$today){echo "(".date('d-m-Y',strtotime($date)).")";}?></td>
