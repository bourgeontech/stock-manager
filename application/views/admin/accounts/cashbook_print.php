...
$payment = $this->db->query("SELECT sum(amount) as payment FROM `payment` WHERE `payment_date` = '$convertDate' and type='1'")->result_array();
  ...
  word-break: break-word;width:350px;"><?php echo $val['led_mal'];?><br><?php echo $val['narration']?></td>
                		    	...
                		    	<tbody>
                	    			...
                	    			foreach ($query3 as $val){
                	    			    ...
                	    			    <tr>
                	    			    	...
                	    			    	<td style="white-space: normal;
  word-wrap: break-word;
  word-break: break-word;width:300px;"><?php echo $val['led_mal'];?><br><?php echo $val['narration']?></td>
                	    			    	<td style="text-align: right;"><?php echo (number_format($cash, 2, '.', ''));?></td>
                	    			    	<td style="text-align: right;"><?php echo (number_format($bank, 2, '.', ''));?></td>
                	    			    </tr>
                	    			    <?php
...
<script>
window.onfocus = function () { setTimeout(function () { window.location = "<?php echo base_url();?>index.php/accounts/cashbook" }, 500); }
function myFunction(){
    window.location = "<?php echo base_url();?>index.php/accounts/cashbook";
}
