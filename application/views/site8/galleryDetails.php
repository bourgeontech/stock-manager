...
.gallery-item {
    flex-basis: 24%; /* Smaller size to fit more images in a row */
    margin-bottom: 10px;
    margin-right: 1%;
    opacity: .85;
    cursor: pointer;
...
<div class="gallery-container m-4">
    <?php
    if (!empty($galleryDetails)) {
        foreach ($galleryDetails as $index => $value) {
            $originalDate = $value['created'];
            $newDate = date("d-m-Y", strtotime($originalDate));
            ?>
            <div class="gallery-item" data-index="<?php echo $index + 1; ?>">
                <img  src="../../../uploads/gallery/<?php echo $value['image']; ?>" >
            </div>
            <?php
        }
    }
...
lightBoxPrev.innerHTML = "&#9664;"; // Left arrow
lightBoxNext.classList.add("lightbox-next");
lightBoxNext.innerHTML = "&#9654;"; // Right arrow
